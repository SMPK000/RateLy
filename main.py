...
def base_name(code: str) -> str:
    return {"TMN":"TMN","IRR":"IRR","USD":"USD","EUR":"EUR","GBP":"GBP","TRY":"TRY","RUB":"RUB","AED":"AED","SAR":"SAR","IQD":"IQD","QAR":"QAR","KWD":"KWD","CAD":"CAD","AUD":"AUD","JPY":"JPY","CHF":"CHF"}.get(code, code)
...
def base_label(code: str, lang: str = "en") -> str:
    return BASE_LABELS.get(lang, BASE_LABELS["en"]).get(code, base_name(code))
...
def home_text(user_id: int) -> str:
    ...
    base = base_label(get_base(user_id), lang)
    group_hint = {
        "fa": "✅ قابل افزودن به گروه‌ها",
        "en": "✅ Can be added to groups",
        "ar": "✅ يمكن إضافته إلى المجموعات",
        "ru": "✅ Можно добавить в группы",
        "tr": "✅ Gruplara eklenebilir",
        "es": "✅ Se puede añadir a grupos",
    }.get(lang, "✅ Can be added to groups")
    return "\n\n".join([
        tr(lang, "welcome"),
...
def assets_text(lang: str) -> str:
    crypto_lines = [
        "BTC, ETH, BNB, SOL, XRP, DOGE, ADA, TON, TRX, LTC, DOT, AVAX, LINK, BCH, ETC",
        "XMR, XLM, ATOM, NEAR, SUI, APT, FIL, ALGO, VET, UNI, AAVE, OP, ARB, INJ, RUNE",
        "ICP, HBAR, SEI, TIA, RENDER, ONDO, FET, TAO, MKR, CRV, SNX, GRT, KAS, PEPE",
        "SHIB, BONK, FLOKI, JUP, WLD, PYTH, JTO, PENDLE, ENS, LDO, ETHFI, ZRO",
    ]
    fiat_lines = [
        "TMN (Toman), IRR (Iranian Rial), USD, EUR, GBP, JPY, CHF, CAD, AUD, NZD",
        "CNY, HKD, SGD, SEK, NOK, DKK, PLN, CZK, HUF, RON, TRY, SAR, AED, QAR, KWD, BHD",
        "INR, PKR, RUB, UAH, ZAR, MXN, BRL, IDR, MYR, THB, PHP, KRW, TWD, ILS, EGP",
        "KZT, VND, ARS, COP, CLP, NGN",
    ]
    metal_lines = [
    ...
    labels = {
        "fa": "🗂 دارایی‌های قابل جستجو",
        "en": "🗂 Searchable assets",
        "ar": "🗂 الأصول القابلة للبحث",
        "ru": "🗂 Активы для поиска",
        "tr": "🗂 Aranabilir varlıklar",
        "es": "🗂 Activos buscables",
    }
    note = {
        "fa": "نام‌های رایج و نمادها هم پشتیبانی می‌شوند؛ اگر اسمش را بفرستی هم تلاش می‌کند پیدا کند.",
        "en": "Common names and symbols are searchable too; it will try names, symbols, and pairs.",
        "ar": "الأسماء الشائعة والرموز مدعومة أيضًا؛ سيحاول إيجاد الاسم أو الرمز أو الزوج.",
        "ru": "Поддерживаются и обычные названия, и символы; бот попробует найти имя, символ или пару.",
        "tr": "Yaygın adlar ve semboller de aranır; ad, sembol veya pariteleri dener.",
        "es": "También se buscan nombres y símbolos comunes; intentará nombre, símbolo o par.",
    }
    return "\n\n".join([
...
def ticket_prompt_text(lang: str, kind: str) -> str:
    prompts = {
        ...
        "ads": {
            "fa": "پیشنهاد تبلیغ، شرایط و بودجه‌ی مدنظرت را بفرست تا برای بررسی برای ادمین ثبت شود.",
            "en": "Send your ad offer, terms, and proposed budget. It will be passed to the admin for review.",
            "ar": "أرسل عرض الإعلان والشروط والميزانية المقترحة ليتم إرسالها للمراجعة إلى المدير.",
            "ru": "Отправьте предложение по рекламе, условия и бюджет — это будет передано администратору на рассмотрение.",
            "tr": "Reklam teklifini, koşulları ve bütçeni gönder. İnceleme için yöneticiye iletilir.",
            "es": "Envía tu propuesta publicitaria, condiciones y presupuesto para revisión del admin.",
        },
        "feedback": {
...
def base_keyboard(user_id: int = None):
    ...
    for code in BASE_CURRENCIES:
        row.append(InlineKeyboardButton(base_label(code, lang), callback_data=f"base:{code}"))
        if len(row) == 3:
            rows.append(row); row = []
    if row: rows.append(row)
    rows.append([InlineKeyboardButton(button_label(lang, "back"), callback_data="menu:back")])
...
def fiat_pair_rate(base: str, quote: str) -> Optional[float]:
    ...
    if base == quote:
        return 1.0
    br = get_base_rate_to_usd(base)
    qr = get_base_rate_to_usd(quote)
    if br and qr:
        return qr / br
    return None

def resolve_crypto_candidate(q: str) -> Tuple[Optional[str], Optional[float]]:
    s = q.strip().upper()
    if s in CRYPTO_ALIASES:
        return s, get_crypto_price_usd(s)
    found = coin_gecko_search(q)
    if found and found.get("id"):
        symbol = (found.get("symbol") or q).upper()
        price = get_crypto_price_usd(found["id"])
        if price is not None:
            return symbol, price
    price = get_crypto_price_usd(q)
    if price is not None:
        return s or q.upper(), price
    return None, None

def resolve_metal_candidate(q: str) -> Tuple[Optional[str], Optional[float], Optional[str]]:
    s = q.strip().upper()
    if s in {"XAU", "GOLD"}:
        for t in ["GC=F", "XAUUSD=X", "GLD"]:
            ...
            if px:
                return "XAU", px, t
        return "XAU", None, "GC=F"
    if s in {"XAG", "SILVER"}:
        for t in ["SI=F", "XAGUSD=X", "SLV"]:
            px = yfinance_last_close(t)
            if px:
                return "XAG", px, t
        return "XAG", None, "SI=F"
    if s in {"XPT", "PLATINUM"}:
        for t in ["PL=F", "XPTUSD=X"]:
            px = yfinance_last_close(t)
            if px:
                return "XPT", px, t
        return "XPT", None, "PL=F"
    if s in {"XPD", "PALLADIUM"}:
        for t in ["PA=F", "XPDUSD=X"]:
            px = yfinance_last_close(t)
            if px:
                return "XPD", px, t
        return "XPD", None, "PA=F"
    return None, None, None

def resolve_asset(query: str) -> Tuple[Optional[str], Optional[float], Optional[str], Optional[str], Optional[str]]:
    phrase = resolve_phrase(query)
    if not phrase:
        return None, None, None, None, None

    q = phrase.upper().replace("-", "/")
    q_compact = q.replace(" ", "")

    metal_symbol, metal_price, metal_ticker = resolve_metal_candidate(q_compact)
    if metal_symbol:
        return metal_ticker, metal_price, "metal", metal_symbol, "USD"

    if q_compact in CRYPTO_ALIASES:
        return q_compact, get_crypto_price_usd(q_compact), "crypto", q_compact, "USD"

    base, quote = parse_pair(q_compact)
    if quote:
        ...
        c_sym, c_price = resolve_crypto_candidate(phrase)
        if c_price is not None:
            return c_sym or base, c_price, "crypto", c_sym or base, quote
        t = f"{base}{quote}=X"
        px = yfinance_last_close(t)
        if px:
    ...
    if q_compact == "TMN":
        rate = get_base_rate_to_usd("TMN")
        return "TMN", (1 / rate) if rate else None, "fiat", "TMN", "USD"

    if q_compact in FIAT_CODES:
        if q_compact == "USD":
            return "USD", 1.0, "fiat", "USD", "USD"
        rate = get_base_rate_to_usd(q_compact)
        return q_compact, (1 / rate) if rate else None, "fiat", q_compact, "USD"

    c_sym, c_price = resolve_crypto_candidate(phrase)
    if c_price is not None:
        return c_sym or q_compact, c_price, "crypto", c_sym or q_compact, "USD"

    # broad yfinance fallback for tickers / commodities / indices
    for t in [q_compact, f"{q_compact}=X", f"{q_compact}-USD", f"{q_compact}USD=X"]:
        px = yfinance_last_close(t)
        if px:
            return t, px, "other", q_compact, "USD"
    return None, None, None, None, None

def money_text(value: float, code: str, lang: str) -> str:
    unit = base_label(code, lang)
    if code in {"TMN", "IRR"}:
        return f"{value:,.0f} {unit}"
    return f"{value:,.4f} {unit}" if abs(value) < 1000 else f"{value:,.2f} {unit}"
...
def format_price_response(query: str, lang: str, base_currency: str) -> str:
    ...
    if kind == "crypto":
        ...
        if disp is not None:
            lines.append(f"{base_label(base_currency, lang)}: {money_text(disp, base_currency, lang)}")
        ...
        if disp is not None:
            lines.append(f"{base_label(base_currency, lang)}: {money_text(disp, base_currency, lang)} per ounce")
    else:
        lines.append(f"USD: {format_usd(usd_price)}")
        if disp is not None:
            lines.append(f"{base_label(base_currency, lang)}: {money_text(disp, base_currency, lang)}")
...
def chart_symbol_for(query: str) -> Optional[str]:
    ticker, _, kind, base, quote = resolve_asset(query)
    if ticker is None:
        return None
    if kind == "crypto":
        sym = ticker.upper().replace(" ", "")
        return sym if sym.endswith("-USD") or sym.endswith("=X") else f"{sym}-USD"
    if kind in {"fiat", "pair", "metal", "commodity", "other"}:
        return ticker
    return ticker
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    lang = get_lang(uid)
    group_note = {
        "fa": "✅ می‌توانی ربات را به گروه هم اضافه کنی.",
        "en": "✅ You can add the bot to groups too.",
        "ar": "✅ يمكنك إضافة البوت إلى المجموعات أيضًا.",
        "ru": "✅ Бота можно добавить и в группы.",
        "tr": "✅ Botu gruplara da ekleyebilirsin.",
        "es": "✅ También puedes añadir el bot a grupos.",
    }.get(lang, "✅ You can add the bot to groups too.")
    await update.effective_message.reply_text(
        f"{tr(lang, 'help')}\n\n{tr(lang, 'quick')}\n\n{group_note}",
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=main_keyboard(uid),
